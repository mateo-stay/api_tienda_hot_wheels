...
SECRET_KEY = "super_clave_ultra_secreta"  # cámbiala en producción
ALGORITHM = "HS256"
...
def verificar_token(
  ...
  try:
    ...
    # payload tiene "sub" (email), "role", opcional "name"
    return payload
  except JWTError:
    raise HTTPException(
      status_code=401,
...
def login(data: schemas.LoginRequest, db: Session = Depends(get_db)):
  ...
  # Añadimos también el nombre al token (opcional, pero útil)
  token = crear_token(
    {
      "sub": usuario.email,
      "role": usuario.rol,
  ...
  # Devolvemos token + info de usuario para el frontend
  return {
    "token": token,
    "usuario": {
      "id": usuario.id,
...
# (Opcional) endpoint para saber quién soy a partir del token
...
# POST /api/productos - crear (solo autenticado; idealmente admin)
...
def crear_producto(
  ...
  # Si quieres dejarlo solo para admin, descomenta:
  # if user.get("role") != "admin":
...
# PUT /api/productos/{producto_id} - actualizar (solo autenticado)
...
# DELETE /api/productos/{producto_id} - eliminar (solo autenticado)
